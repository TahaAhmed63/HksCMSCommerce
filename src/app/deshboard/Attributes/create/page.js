...
async function deleteAttribute(attributeId) {
  try {
    const response = await fetch(`http://localhost:3000/api/attributes/${attributeId}`, {
      method: 'DELETE',
    });
          ...
          <TableBody>
            {attributes.map(attribute => (
              <TableRow key={attribute.id}>
                ...
                <TableCell className="cursor-pointer	"  onClick={() => handleAttributeClick(attribute.id)}>{attribute.name}</TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    <Button variant="ghost" size="icon"  onClick={() => handleAttributeClick2(attribute.id)} >
